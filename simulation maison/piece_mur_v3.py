...
def animation_diff(lamb_mur, rho_mur, c_mur, lamb_air, rho_air, c_air, h_cc, epaisseur, hauteur, T_haut, T_bas, T_droite, T_gauche, T_int_mur, T_int_piece, x_piece, y_piece, chauffage=False):
    ...
    def calculate_piece(u, u1, u2, u3, u4, chauffage):
        if chauffage == False:  # test pour savoir si la piece est chauffée
            ...
            # Calcul de la temperature en tout points de la piece en tenant compte du chauffage
    ...
    ax_piece = fig.add_subplot(gs[1, 1])
    plt.xlabel("x")
    plt.ylabel("y")
    ax_piece.set_aspect('equal')

    ax_mur1 = fig.add_subplot(gs[0, 1])
    ax_mur1.set_aspect('equal')

    ax_mur2 = fig.add_subplot(gs[2, 1])
    ax_mur2.set_aspect('equal')

    ax_mur3 = fig.add_subplot(gs[1, 0])
    ax_mur3.set_aspect('equal')

    ax_mur4 = fig.add_subplot(gs[1, 2])
    ax_mur4.set_aspect('equal')

    def plotheatmap(piece_k, mur1_k, mur2_k, mur3_k, mur4_k, k):
        # Actualise le temps
        #ax_piece.title(f"Temperature at t = {k*delta_t:.3f} unit time")
        # Plot la matrice
        ax_piece.pcolormesh(piece_k, cmap=plt.cm.jet, vmin=0, vmax=100)
        ax_mur1.pcolormesh(mur1_k, cmap=plt.cm.jet, vmin=0, vmax=100)
        ax_mur2.pcolormesh(mur2_k, cmap=plt.cm.jet, vmin=0, vmax=100)
        ax_mur3.pcolormesh(mur3_k, cmap=plt.cm.jet, vmin=0, vmax=100)
        ax_mur4.pcolormesh(mur4_k, cmap=plt.cm.jet, vmin=0, vmax=100)
        # ax.colorbar()
        return fig
    ...
    def animate(k):
        plotheatmap(res_piece[k], res_mur1[k],
                    res_mur2[k], res_mur3[k], res_mur4[k], k)
