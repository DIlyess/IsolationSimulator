...
def animation_diff(lamb, rho, c, epaisseur, hauteur, T_haut, T_bas, T_droite, T_gauche, T_int_mur, T_int_piece, x_piece, y_piece, T_chauffage=30, chauffage=False):
    ...
    res_fenetre = np.zeros((max_iter_time, hauteur, epaisseur))

    # Initialise la température du mur
    res_mur.fill(T_int_mur)
    res_fenetre.fill(T_int_mur)
    ...
    res_fenetre[:, :, (epaisseur-1):] = T_droite
    res_fenetre[:, :, :1] = T_gauche
    res_fenetre[:, :1, :] = T_bas
    res_fenetre[:, (hauteur-1):, :] = T_haut

    # Résolution avec la méthode des différences finies pour le mur
    def calculate_mur(u, gamma):
    ...
    res_mur = calculate_mur(res_mur, gamma_mur)
    res_fenetre = calculate_mur(res_fenetre, gamma_fenetre)

    # Récolte les températures au bord du mur côté pièce pour chaque temps k
    T_bord_mur = res_mur[:, hauteur//2, epaisseur//2]
    T_bord_fenetre = res_fenetre[:, hauteur//2, epaisseur//2]
    ...
    for k in range(max_iter_time):
        ...
        if fenetre:
            res_piece[k, :1, 1:20] = T_bord_mur[k]
            res_piece[k, :1, 20:30] = T_bord_fenetre[k]
            res_piece[k, :1, 30:] = T_bord_mur[k]
        else: 
            res_piece[k, :1, 1:] = T_bord_mur[k]
        res_piece[k, :, (x_piece-1):] = T_bord_mur[k]
<<<<<<< HEAD

    print(T_bord_fenetre[700])
    print(T_bord_mur[700])
=======
>>>>>>> ec2cf13658dd09bd6608957420b4a5ba5dee5f44
    ...
    if fenetre:
        tracer_maison(Maison1_fenetre)
    else:
        tracer_maison(Maison1)
    plt.xlabel("x")
    plt.ylabel("y")
